...
<!--{/if}-->
<div class="alert alert-warning">平台用户总余额：{$usermoney}，目前通过微信绑定账号总人数：{$bindopenidusernum}人，目前已经激活邮箱的总人数:{$emailactiveusernum}人</div>
                ...
                <td  width="20%">用户组：
                 ...
                 <td  width="20%">条件刷选：
                    <select name="ischeck" class="form-control">
                       ...
                       <option value="3" {if isset($search['isecheck'])&&$search['isecheck'] == 3}selected{/if}>--邮箱微信验证--</option>
                        <option value="4" {if isset($search['isecheck'])&&$search['isecheck'] == 4}selected{/if}>--账户金额大于0--</option>
                    </select>
                </td>
...
<form name="userForm" action="index.php?admin_user/remove{$setting['seo_suffix']}" method="post">
    <table class="table table-striped">
        ...
        <tr>
            ...
            <td class="altbg2">{echo $member['jine']/100;}元</td>
            ...
            <td class="altbg2">{if $member['openid']}已绑定<span class="hand" onclick="thankTa({$member['uid']})">【感谢Ta注册】</span>{/if}</td>
