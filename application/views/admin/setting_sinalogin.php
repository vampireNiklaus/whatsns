...
<form action="index.php?admin_setting/sinalogin{$setting['seo_suffix']}" method="post">
    <table class="table">
        ...
        <tr>
            <td width="45%"><b>是否自动下载sina微博头像：</b><br><span class="smalltxt">设置之后将设置用户头像为sina微博头像，会导致sina登陆速度较慢。</span></td>
            <td>
                <input type="radio" value="1" name="sinalogin_avatar" {if $setting['sinalogin_avatar']}checked{/if}/>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="0" name="sinalogin_avatar" {if !$setting['sinalogin_avatar']}checked{/if}/>否
            </td>
        </tr>
