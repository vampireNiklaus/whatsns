...
<form name="askform" action="index.php?admin_category/edit{$setting['seo_suffix']}" method="post"  enctype="multipart/form-data">
    ...
    <table class="table">
        ...
        <option value="catlist" {if $category['template']=='catlist'}selected{/if}>Application/view/$setting['tpl_dir']/catlist.html</option>
        <option value="catlist_topic" {if $category['template']=='catlist_topic'}selected{/if}>Application/view/$setting['tpl_dir']/catlist_topic.html</option>
        <option value="catlist_text" {if $category['template']=='catlist_text'}selected{/if}>Application/view/$setting['tpl_dir']/catlist_text.html</option>
        </select>
      <h5>说明：</h5>
      <p>catlist.html：文章默认模板，列表由封面图+标题+描述组合，图片靠右边展示</p>
       <p>catlist_topic.html：列表由封面图+标题组合，图片靠上展示</p>
        <p>catlist_text.html：列表只显示标题</p>
        <div style="color:red">
           {if $category['template']==null||$category['template']==''}
                              当前应用模板：Application/view/$setting['tpl_dir']/catlist.html
                          {else}
                                                         当前应用模板：Application/view/$setting['tpl_dir']/$category['template'].html
                           {/if}
        </div>
            </td>
        </tr>
