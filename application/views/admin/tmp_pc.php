...
<div class="alert alert-warning">Linux主机如需在线修改模板，请保证application/views/ 全部文件夹可以读写777(admin文件夹除外)</div>
<table class="table tmp_table"  align="left" >
    <tr class="tp_tr">
...
<table  class=" table tableborder tplist">
    <thead >
      <th align='center'>模板序号</th>
    <th align='center'>模板文件</th>
    <th align='center'>模板文件备注</th>
...
<script>
    var selectdir="";
    var fileurl=g_site_url+"index.php?admin_template/getpcdir{$setting['seo_suffix']}";
    $.ajax({
        url: fileurl,
        dataType: 'text',
            ...
            if(temps.length>0){
                ...
                for(var tp=0;tp<temps.length;tp++){
                    if(temps[tp]!=''){
                        ...
                        var  lifile="<li class='urlcollect' ><a style='cursor:pointer'>val</a></li>";
                ...
                $(".tpldir li").css("position","relative").css("left",'-30px').css("text-align","center");
