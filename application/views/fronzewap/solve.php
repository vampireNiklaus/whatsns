  ...
  <!--{/if}-->
        ...
        <div class="article-content">
         <div class="ask_detail_content_text qyer_spam_text_filter">
            ...
            <div class="show-foot">
                <a class="notebook" href="{url category/view/$question['cid']}" data-toggle="tooltip" data-html="true" data-original-title="文章归属分类">
                    <i class="fa fa-file-text"></i> <span>{$question['category_name']}</span>
                </a>          <div class="copyright" data-toggle="tooltip" data-html="true" data-original-title="举报问题">
                <a onclick="openinform({$question['id']},'{$question['title']}',0)"  id="report-modal">举报问题</a>
            </div>
                <div class="modal-wrap" data-report-note="">
                     <!--{if $is_followed}-->
                      <a href="{url question/attentto/$question['id']}">已收藏</a>
                         <!--{else}-->
                          <a href="{url question/attentto/$question['id']}">收藏</a>
                          <!--{/if}-->
                </div>
            </div>
    </article>
       <!--{if 0!=$question['shangjin']}-->
      <div style="background-color: #867775;    border-radius: .15em;color:#fff;font-size:12px;padding:.05rem .85em;margin:0 .85em;margin-top:10px;">此问题作者打赏 <span style="font-size:15px;">$question['shangjin']</span> 元，如果回答被采纳将会将赏金放入您平台账户钱包，您可以提现到微信零钱里。</div>
          <!--{/if}-->
    <!--回答-->
         ...
         <div class="appendcontent font-12">
                    ...
                    <div class="operationlist">
                     ...
                     <i class="ui-icon-comment"></i>
                      ...
                      {if $signPackage!=null}
                            <span onclick="mobilebestcallpay({$bestanswer['id']},{$bestanswer['authorid']},{$setting['mobile_shang']})" >
   ...
   <div class="appendcontent">
                    ...
                    <div class="ans-footer">
                     ...
                     <i class="ui-icon-comment"></i>
                        ...
                        {if $signPackage!=null}
                            <span onclick="mobilebestcallpay({$answer['id']},{$answer['authorid']},{$setting['mobile_shang']})" >
...
<div id="list-container">
 ...
 <li id="note-{$solve['id']}" data-note-id="{$solve['id']}" class="">
            ...
            <div class="content">
                <div class="author">
                    ...
                    <a class="avatar" target="_self" href="{url user/space/$solve['authorid']}">
                        <img src="{$solve['avatar']}" alt="96" class="lazy" style="display: inline;">
                    </a>      <div class="name">
                    <a class="blue-link" target="_self" href="{url user/space/$solve['authorid']}">{$solve['author']} {if $solve['author_has_vertify']!=false}<i class="fa fa-vimeo {if $solve['author_has_vertify'][0]=='0'}v_person {else}v_company {/if}  " data-toggle="tooltip" data-placement="right" title="" {if $solve['author_has_vertify'][0]==0}data-original-title="个人认证" {else}data-original-title="企业认证" {/if} ></i>{/if} </a>




                    <span class="ico_point">.</span>
                    <span class="time" data-shared-at="{$solve['format_time']}">{$solve['format_time']}</span>
                </div>
                </div>
                <a class="title" target="_self" href="{url question/view/$solve['id']}">{$solve['title']}</a>
                <p class="abstract">
                {eval echo clearhtml($solve['description']);}

                </p>
               <div class="meta">
                <a class="collection-tag" target="_self" href="{url category/view/$solve['cid']}  ">$solve['category_name']</a>
                <a target="_self"   href="{url question/view/$solve['id']}" >
                    <i class="fa fa-eye"></i> {$solve['views']}
                </a>        <a target="_self"  href="{url question/view/$solve['id']}#comments" >
                <i class="fa fa-comment-o"></i> {$solve['answers']}
            </a>      <span><i class=" fa fa-heart-o"></i>  {$solve['attentions']}</span>
            </div>
            </div>
        </li>
