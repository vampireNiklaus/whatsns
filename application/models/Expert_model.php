...
class Expert_model extends CI_Model {
	...
	function get_solves($start = 0, $limit = 20) {
		$solvelist = array ();
		$query = $this->db->query ( "SELECT distinct a.qid,a.title FROM " . $this->db->dbprefix . "answer  as a ,`" . $this->db->dbprefix . "user_category` as f WHERE a.authorid=f.uid ORDER BY a.time DESC LIMIT $start ,$limit" );
