...
class Doing_model extends CI_Model {
	...
	var $actiontable = array ('1' => '提出了问题', '2' => '回答了该问题', '3' => '评论该回答', '4' => '关注了该问题', '5' => '赞同了该回答', '6' => '对该回答进行了追问', '7' => '继续回答了该问题', '8' => '采纳了回答', '9' => '发布了文章', '10' => '关注了专题', '11' => '关注了用户', '12' => '注册了网站', '13' => '收藏了文章', '14' => '评论了文章', '15' => '付费阅读了文章' );
	...
	function add($authorid, $author, $action, $qid, $content = '', $referid = 0, $refer_authorid = 0, $refer_content = '') {
		...
		$data=array('doingid'=>NULL,'authorid'=>$authorid,'author'=>$author,'action'=>$action,'questionid'=>$qid,'content'=>$content,'referid'=>$referid,'refer_authorid'=>$refer_authorid,'refer_content'=>$refer_content,'createtime'=>time());
	...
	function list_by_type($searchtype = 'all', $uid = 0, $start = 0, $limit = 20) {
		...
		switch ($searchtype) {
			case 'all' :
				$sql .= "select * from " . $this->db->dbprefix . "doing ";
				break;
			case 'my' :
	...
	function deletedoing($uid, $type, $typeid) {
		$this->db->where ( array ('authorid' => $uid, 'action' => $type, 'questionid' => $typeid ) )->delete ( 'doing' );
	}
	/* 获取问题管理者列表信息 */

	function get_follower($qid, $start = 0, $limit = 16) {
		$followerlist = array ();
		$query=$this->db->where ( array ('qid' => $qid ) )->order_by ( 'time DESC' )->limit ( $limit, $start )->get ( 'question_attention' );
	...
	function rownum_by_type($searchtype = 'all', $uid = 0) {
		...
		switch ($searchtype) {
			...
			case 'my' :
				...
				break;
			case 'atentto' :
				$sql .= "select count(d.questionid) as num  from " . $this->db->dbprefix . "doing as d," .  $this->db->dbprefix . "user_attention as u where d.authorid=u.followerid and u.uid=$uid and action in(1,2,3,6,9,11)";
				break;
			default :
				$sql .= "select count(questionid) as num  from " . $this->db->dbprefix . "doing ";
				break;
