...
class User_model extends CI_Model {
	...
	function get_by_uid($uid, $loginstatus = 1) {
		...
		$user ['author_has_vertify'] = isset ( $user ['uid'] ) && get_vertify_info ( $user ['uid'] ); //用户是否认证


		$user ['grouptitle'] = isset ( $user ['groupid'] ) && $usergroup [$user ['groupid']] ['grouptitle'];
	...
	function getallusermoney() {
		...
		$query = $this->db->select_sum ( 'jine', 'sum' )->where ( array ('jine>' => 0 ) )->get ( 'user' );
	...
	function getsumuserbyopenid() {
		$this->db->select ( array ('openid', 'uid' ) )->where ( array ('openid!=' => '' ) )->from ( 'user' );
	...
	function getsumuserbyueamactive() {
		...
		$this->db->where ( array ('active' => 1 ) )->from ( 'user' );

		return $this->db->count_all_results ();
	}
	function get_by_username($username) {

		$query = $this->db->where ( array ('username' => $username ) )->or_where ( array ('email' => $username ) )->or_where ( array ('phone' => $username ) )->from ( 'user' )->get ();
	...
	function get_active_list($start = 0, $limit = 10, $cid = 'all', $status = 'all') {
		global $user;
		$userlist = array ();
		$orderwhere = array ('1' => 1 );
		switch ($status) {
			case 'all' : //全部
				$orderwhere = array ('1' => 1 );
				break;
			case '1' : //付费
				$orderwhere = array ('mypay>' => 0 );
				break;
			case '2' : //免费
				$orderwhere = array ('mypay' => 0 );
				break;
	...
	function get_caiji_list($start = 0, $limit = 10) {
		...
		$query = $this->db->select ( '*' )->from ( 'user' )->where ( array ('fromsite' => 1 ) )->order_by ( 'lastlogin desc' )->limit ( $limit, $start )->get ();
	...
	function get_active_list_bynosign($start = 0, $limit = 10) {
		$userlist = array ();
		$query = $this->db->select ( '*' )->from ( 'user' )->where ( array ('signature' => '这个人很懒，什么都没留下' ) )->or_where ( array ('signature' => null ) )->order_by ( 'lastlogin desc,credit2 DESC' )->limit ( $limit, $start )->get ();
	...
	function list_by_credit($type = 0, $limit = 100) {
		...
		$sqlarray = array ('SELECT u.uid,u.groupid, u.username,u.gender,u.lastlogin,u.credit2,u.questions,u.answers,u.adopts FROM ' . $this->db->dbprefix . "user  u ORDER BY `credit2` DESC,u.answers DESC  LIMIT 0,$limit", "SELECT u.uid,u.groupid, u.username,u.gender,u.lastlogin,sum( c.credit2 ) credit2,u.questions,u.answers,u.adopts FROM " . $this->db->dbprefix . "user u," . $this->db->dbprefix . "credit c   WHERE u.uid=c.uid AND c.time>$starttime   GROUP BY u.uid ORDER BY credit2  DESC,u.answers DESC LIMIT 0,$limit", "SELECT u.uid,u.groupid, u.username,u.gender,u.lastlogin,sum( c.credit2 ) credit2,u.questions,u.answers,u.adopts  FROM " . $this->db->dbprefix . "user u," . $this->db->dbprefix . "credit c   WHERE u.uid=c.uid AND c.time>$starttime   GROUP BY u.uid ORDER BY credit2  DESC,u.answers DESC LIMIT 0,$limit" );
	...
	function refresh($uid, $islogin = 1, $cookietime = 0) {
		...
		$query = $this->db->select ( '*' )->from ( 'user' )->join ( 'usergroup', 'usergroup.groupid=user.groupid' )->where ( array ('user.uid' => $uid ) )->get ();
		$user = $query->row_array ();

		$this->db->set ( 'lastlogin', time () )->where ( array ('uid' => $uid ) )->update ( 'user' );
		$this->db->where ( array ('uid' => $uid ) )->delete ( 'session' );
		$data = array ('sid' => $sid, 'uid' => $uid, 'islogin' => $islogin, 'ip' => getip (), 'time' => time () );
	...
	function refresh_session_time($sid, $uid) {
		...
		if (! $lastrefresh) {
			if ($uid) {
				...
				$query = $this->db->get_where ( 'session', array ('sid' => $sid ) );
				...
				if ($session) {
					...
					$data = array ('sid' => $sid, 'ip' => $this->input->ip_address (), 'time' => time () );
	...
	function add($username, $password, $email = '', $uid = 0) {
		...
		if ($uid) {
			$data = array ('uid' => $uid, 'username' => $username, 'password' => $password, 'email' => $email, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time () );
	...
	function caijiadd($username, $password, $email = '', $fromsite = 1) {
		$password = md5 ( $password );
		$data = array ('username' => $username, 'password' => $password, 'fromsite' => $fromsite, 'email' => $email, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time () );
		$this->db->insert ( 'user', $data );
		$uid = $this->db->insert_id ();
	...
	function weixinadd($username, $password, $openid = '') {
		$password = md5 ( $password );
		$data = array ('username' => $username, 'password' => $password, 'openid' => $openid, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time () );
		$this->db->insert ( 'user', $data );
	...
	function adduserapi($username, $password, $email = '', $groupid = 7, $uid = 0, $phone = 0) {
		$password = md5 ( $password );
		if ($uid) {
			$data = array ('uid' => $uid, 'username' => $username, 'password' => $password, 'email' => $email, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time (), 'groupid' => $groupid );
			$this->db->replace ( 'user', $data );
		} else {
			if ($phone == 0) {
				$phone = '';
				$data = array ('username' => $username, 'password' => $password, 'email' => $email, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time (), 'groupid' => $groupid, 'phone' => $phone );
				$this->db->insert ( 'user', $data );
			} else {
				$data = array ('username' => $username, 'password' => $password, 'email' => $email, 'regip' => getip (), 'regtime' => time (), 'lastlogin' => time (), 'groupid' => $groupid, 'phone' => $phone, 'phoneactive' => 1 );
				$this->db->insert ( 'user', $data );
			}
	...
	function updateinvatecode($uid, $frominvatecode) {
		...
		if ($user) {
			//更新用户信息
			$this->db->update ( 'user', array ('frominvatecode' => $frominvatecode ), array ('uid' => $uid ) );
			$invateusers = intval ( $user ['invateusers'] ) + 1; //邀请人数+1
			$this->db->update ( 'user', array ('invateusers' => $invateusers ), array ('uid' => $user ['uid'] ) );
			//将邀请人自动关注到被邀请人
	...
	function sendinvatecodetouid($uid) {
		//随机产生一个邀请码
		$invatecode = rand ( 2000000, 999999999 );
		//检查有没有这个人，如果有就分配
		$user = $this->db->query ( "SELECT * FROM " . $this->db->dbprefix . "user WHERE invatecode='$invatecode'" )->row_array ();
		if (! $user) {
			//更新用户信息
			$this->db->update ( 'user', array ('invatecode' => $invatecode ), array ('uid' => $uid ) );
			$this->refresh ( $uid );
		}
	}
	...
	function is_allowed_register() {
		...
		$this->db->from ( 'user' )->where ( array ('regip' => getip (), 'regtime>' => $starttime, 'regtime<' => $endtime ) );
	...
	function uppass($uid, $password) {
		$password = md5 ( $password );
		$this->db->set ( 'password', $password )->where ( array ('uid' => $uid ) )->update ( 'user' );
	...
	function update($uid, $gender, $bday, $phone, $qq, $msn, $introduction, $signature, $isnotify = 1) {
		$data = array ('gender' => $gender, 'bday' => $bday, 'phone' => $phone, 'qq' => $qq, 'msn' => $msn, 'introduction' => $introduction, 'signature' => $signature, 'isnotify' => $isnotify );
		$this->db->set ( $data )->where ( array ('uid' => $uid ) )->update ( 'user' );
	}

	function update_email($email, $uid) {
		$this->db->set ( 'email', $email )->where ( array ('uid' => $uid ) )->update ( 'user' );
	}
	/*修改邮箱并修改激活为0 */
	function update_emailandactive($email, $activecode, $uid) {
		$data = array ('activecode' => $activecode, 'active' => 0, 'email' => $email );
		$this->db->set ( $data )->where ( array ('uid' => $uid ) )->update ( 'user' );

	}
	/*修改激活为1,邮箱激活 */
	function update_useractive($uid) {
		$this->db->set ( 'active', 1 )->where ( array ('uid' => $uid ) )->update ( 'user' );

	}
	/*激活用户手机号码*/
	function updatephone($uid, $userphone) {
		$data = array ('phone' => $userphone, 'phoneactive' => 1 );
		$this->db->set ( $data )->where ( array ('uid' => $uid ) )->update ( 'user' );
	...
	function update_gift($uid, $realname, $email, $phone, $qq) {
		$data = array ('realname' => $realname, 'email' => $email, 'phone' => $phone, 'qq' => $qq );
		$this->db->set ( $data )->where ( array ('uid' => $uid ) )->update ( 'user' );
	...
	function update_user($uid, $username, $passwd, $email, $groupid, $credits, $credit1, $credit2, $gender, $bday, $phone, $qq, $msn, $introduction, $signature, $isblack = 0) {
		...
		$data = array ('username' => $username, 'password' => $passwd, 'isblack' => $isblack, 'email' => $email, 'groupid' => $groupid, 'credits' => $credits, 'credit1' => $credit1, 'credit2' => $credit2, 'gender' => $gender, 'bday' => $bday, 'phone' => $phone, 'qq' => $qq, 'msn' => $msn, 'introduction' => $introduction, 'signature' => $signature );

		$this->db->where ( array ('uid' => $uid ) )->update ( 'user', $data );
	...
	function update_username($uid, $username, $useremail) {
		$data = array ('username' => $username, 'email' => $useremail );
		$this->db->set ( $data )->where ( array ('uid' => $uid ) )->update ( 'user' );
	}
	...
	function remove($uids, $all = 0) {
		...
		/* 删除问题和回答,文章 */
		if ($all) {
			$tables = array ('question', 'topic', 'ask_articlecomment', 'answer' );
	...
	function logout() {
		...
		tcookie ( 'auth', '', 0 );
		tcookie ( 'loginuser', '', 0 );
		$query = $this->db->select_max ( 'time' )->where ( array ('uid' => $user ['uid'] ) )->get ( 'session' );
		$lasttime = $query->row_array ();
		$this->db->where ( array ('uid' => $user ['uid'], 'time<' => $lasttime ['time'] ) )->delete ( 'session' );
	}
	...
	function is_login($uid = 0) {
		...
		$query = $this->db->select ( array ('islogin' ) )->get_where ( 'session', array ('uid' => $uid, 'time>' => $onlinetime ) );
	...
	function get_refresh_time($uid) {
		$query = $this->db->from ( 'session' )->where ( array ('uid' => $uid ) )->order_by ( 'time DESC' )->get ();
	...
	function credit_detail($uid, $start = 0, $limit = 10) {
		$details = array ();
		$query = $this->db->where ( array ('uid' => $uid ) )->order_by ( 'time desc' )->limit ( $limit, $start )->get ( 'credit' );
		foreach ( $query->result_array () as $credit ) {
			...
			switch ($credit ['operation']) {
				...
				case 'user/register' :
					...
					break;
				case '支付宝充值' :
					$credit ['content'] = '支付宝积分充值';
					break;
				case '积分充值' :
					$credit ['content'] = '微信积分充值';
					break;
				case 'message/send' :
					$credit ['content'] = '发送私信';
					break;
	...
	function add_category($cid, $uid) {
		$data = array ('cid' => $cid, 'uid' => $uid );
		$this->db->insert ( 'user_category', $data );
	}

	function get_category($uid) {

		$query = $this->db->select ( '*' )->from ( 'user_category' )->where ( array ('uid' => $uid ) )->get ();
	...
	function remove_category($cid, $uid) {
		$this->db->where ( array ('cid' => $cid, 'uid' => $uid ) )->delete ( 'user_category' );
	...
	function get_login_auth($uid, $type = 'qq') {
		$query = $this->db->get_where ( 'login_auth', array ('type' => $type, 'uid' => $uid ) );
		return $query->row_array ();

	}

	function remove_login_auth($uid, $type = 'qq') {
		$this->db->where ( array ('type' => $type, 'uid' => $uid ) )->delete ( 'login_auth' );
	...
	function rownum_alluser() {
		return array ($this->db->count_all ( 'user' ) );
	}
	...
	function rownum_onlineuser() {
		...
		$this->db->where ( array ('time>' => $end ) )->from ( 'session' );
		$this->db->group_by ( "ip" );
		return $this->db->count_all_results ();
	}
	...
	function list_online_user($start = 0, $limit = 50) {
		...
		$onlinelist = array ();
		$end = time () - intval ( $this->setting ['sum_onlineuser_time'] ) * 60;
		$this->db->select ( array ('session.uid', 'session.ip', 'user.username', 'session.time' ) );
		$this->db->from ( 'session' );
		$this->db->join ( 'user', "session.uid = user.uid " );
		$this->db->where ( array ('session.time>' => $end ) );
	...
	function get_follower($uid, $start = 0, $limit = 20) {
		...
		$query = $this->db->select ( '*' )->from ( 'user_attention' )->where ( array ('uid' => $uid ) )->order_by ( 'time DESC' )->limit ( $limit, $start )->get ();
	...
	function getinvatelist($invatecode, $start = 0, $limit = 20) {
		...
		$query = $this->db->select ( '*' )->from ( 'user' )->where ( array ('frominvatecode' => $invatecode ) )->order_by ( 'uid DESC' )->limit ( $limit, $start )->get ();
	...
	function get_attention($followerid, $start = 0, $limit = 20) {
		...
		$attentionlist = array ();
		$this->db->select ( array ('user.uid', 'user.username', 'user.gender' ) );
	...
	function get_attention_category($followerid, $start = 0, $limit = 20) {
		...
		$query = $this->db->select ( '*' )->from ( 'categotry_follower' )->where ( array ('uid' => $followerid ) )->order_by ( 'time DESC' )->limit ( $limit, $start )->get ();
	...
	function rownum_attention_category($followerid) {
		...
		$this->db->from ( 'categotry_follower' )->where ( array ('uid' => $followerid ) );
		return $this->db->count_all_results ();
	...
	function is_followedcid($cid, $uid) {
		$this->db->from ( 'categotry_follower' )->where ( array ('uid' => $uid, 'cid' => $cid ) );
		return $this->db->count_all_results ();
	...
	function is_followed($uid, $followerid) {
		$this->db->from ( 'user_attention' )->where ( array ('uid' => $uid, 'followerid' => $followerid ) );
		return $this->db->count_all_results ();
	...
	function follow($sourceid, $followerid, $follower, $type = 'question') {
		...
		$data = array ("$sourcefield" => $sourceid, 'followerid' => $followerid, 'follower' => $follower, 'time' => time () );

		$this->db->insert ( $type . "_attention", $data );
		if ($type == 'question') {
			$data = array ('qid' => $sourceid, 'uid' => $followerid, 'time' => time () );
	...
	function unfollow($sourceid, $followerid, $type = 'question') {
		...
		$sourcefield = 'qid';
		($type != 'question') && $sourcefield = 'uid';
		$this->db->delete ( $type . "_attention", array ("$sourcefield" => $sourceid, 'followerid' => $followerid ) );
