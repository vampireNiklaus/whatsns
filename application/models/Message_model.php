...
class Message_model extends CI_Model {
	...
	function add($msgfrom, $msgfromid, $msgtoid, $subject, $message) {
		...
		if ($user ['isnotify']) {
			$time = time ();
			$data = array ('from' => $msgfrom, 'fromuid' => $msgfromid, 'touid' => $msgtoid, 'subject' => $subject, 'time' => $time, 'content' => $message );
	...
	function list_by_touid($touid, $start = 0, $limit = 10) {
		$messagelist = array ();
		$touid = intval ( $touid );
		$sql = "SELECT * FROM " . $this->db->dbprefix . "message WHERE touid=$touid AND fromuid!=$touid AND status<>2 AND fromuid=0 ORDER BY `time` DESC LIMIT $start,$limit";
