...
class Api_article extends CI_Controller {
	...
	function clist() {
		...
		$query = $this->db->query ( "SELECT * FROM " . $this->db->dbprefix  . "topic order by id desc LIMIT 0,3" );
			...
			if (strstr ( $topic ['image'], 'http' )) {
				$content [] = array ("Title" => $topic ['title'], "Description" => $description, "PicUrl" => $topic ['image'], "Url" => $this->domain . 'article-' . $topic ['id'].'.html' );
			} else {
				$content [] = array ("Title" => $topic ['title'], "Description" => $description, "PicUrl" => $this->domain . $topic ['image'], "Url" => $this->domain . 'index.php?article-' . $topic ['id'] .'.html');
			}
	...
	function newqlist() {
		...
		$questionlist = array ();
		$query = $this->db->query ( "SELECT * FROM " . $this->db->dbprefix  . "question  order by id desc LIMIT 0,3" );
		...
		foreach ( $query->result_array () as $question ) {
			...
			$question ['url'] = $this->domain . 'index.php?q-' . $question ['id'] . '.html';
	...
	function hotalist() {
		...
		foreach ( $modellist as $key => $val ) {
			...
			if (strstr ( $modellist [$key] ['image'], 'http' )) {
				$content [] = array ("Title" => $modellist [$key] ['title'], "Description" => $description, "PicUrl" => $modellist [$key] ['image'], "Url" => $this->domain . 'article-' . $modellist [$key] ['id'].'.html' );
			} else {
				$content [] = array ("Title" => $modellist [$key] ['title'], "Description" => $description, "PicUrl" => 'http://' . $_SERVER ['HTTP_HOST'] . $modellist [$key] ['image'], "Url" => $this->domain . 'index.php?article-' . $modellist [$key] ['id'].'.html' );
			}
			$i ++;
		}
