...
class Admin_category extends CI_Controller {
	...
	function index($_message = '') {
		...
		foreach ( $tcategorylist as $cat ) {
			if ($cat ['isshowindex'] == null || $cat ['isshowindex'] == 0) {
				$cat ['isshowindex'] = "不应用到首页";
			} else {
				$cat ['isshowindex'] = "应用到首页";
			}
			if ($cat ['isusearticle'] == null || $cat ['isusearticle'] == 0) {
				$cat ['isusearticle'] = "不应用到文章";
			} else {
				$cat ['isusearticle'] = "应用到文章";
			}
	...
	function edit() {
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			cleardir ( FCPATH . '/data/cache' ); //清除缓存文件
			$this->input->post ( 'cid' ) ? $this->myview ( $this->input->post ( 'cid' ) ) : $this->index ( "操作成功" );
		} else {
	...
	function myview($cid = 0, $msg = '') {
		...
		$categorylist = $this->category_model->list_by_cid_pid ( $cid, $category ['pid'] ); //获取子分类
		$pid = $cid;
		$msg && $message = $msg;
		include template ( 'categorylist', 'admin' );
	}
