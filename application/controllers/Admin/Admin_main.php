...
class Admin_main extends CI_Controller {
	...
	function stat() {
		...
		$verifyquestions = returnarraynum ( $this->db->query ( getwheresql ( 'question', '`status`=0', $this->db->dbprefix ) )->row_array () );
		$verifyanswers = returnarraynum ( $this->db->query ( getwheresql ( 'answer', '`status`=0', $this->db->dbprefix ) )->row_array () );
	...
	function regulate() {
		$pagesize = 1000;
		$upagesize = 100;
		$rownum = returnarraynum ( $this->db->query ( getwheresql ( 'user', " 1=1 ", $this->db->dbprefix ) )->row_array () );
	...
	function check_user() {
		...
		$pagesize = 100;
		$startindex = ($page - 1) * $pagesize;
		$query = $this->db->query ( "SELECT * FROM " . $this->db->dbprefix . "user  LIMIT $startindex,$pagesize" );
		foreach ( $query->result_array () as $user ) {
			...
			$this->db->query ( "UPDATE " . $this->db->dbprefix . "user SET articles=$articles,questions=$questions,answers=$answers where uid=" . $user ['uid'] );
		}
		exit ( 'ok' );
	}
