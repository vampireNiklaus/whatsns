...
function show_msg($error_no, $error_msg = 'ok', $success = 1, $quit = TRUE) {
	if(VIEW_OFF) {
		...
		echo '<input class="btn btn-warning" type="button" onclick="history.back()" value="'.lang('click_to_back').'" /><br /><br /><br />';
...
function show_next_step($step, $error_code) {
	...
	if($error_code == 0) {
		$nextstep = "<div class=\"install_btn_group\"><input class=\"btn btn-default\" type=\"button\" onclick=\"history.back();\" value=\"".lang('old_step')."\"><input class=\"btn btn-default\" type=\"submit\" value=\"".lang('new_step')."\"></div>\n";
	} else {
		$nextstep = "<div class=\"install_btn_group\"><input class=\"btn btn-default\" type=\"button\" disabled=\"disabled\" value=\"".lang('not_continue')."\"></div>\n";
	}
...
<button type="submit" class="btn btn-success ">{$lang_agreement_yes}</button>
<button type="button" class="btn btn-success " name="exit" onclick="javascript: window.close(); return false;">{$lang_agreement_no}</button>
</div>
...
<link rel="stylesheet" href="../static/css/install/css/install.css" type="text/css" media="all" />
<link rel="stylesheet" href="../static/css/install/css/style.css" type="text/css" media="all" />
...
<h1><a href="#">whatsns问答系统V$version $install_lang $release</a></h1>
...
function initinput() {
	...
	<div class="main">
		...
		<div class="btnbox marginbot">
	    <input class="btn btn-info" type="submit" name="submit" value="<?=lang('install_in_processed')?>" disabled  id="step3">
	</div>
<?php
}
...
function show_setting($setname, $varname = '', $value = '', $type = 'text|password|checkbox', $error = '') {
	...
	echo "\n".'<tr><th width="100px" class="tbopt'.($error ? ' red' : '').'">&nbsp;'.(empty($setname) ? '' : lang($setname).':')."</th>\n<td>";
...
function show_step($step) {
    ...
    if($stepclass[1]=='current'){
    	$icon1="<i class='fa fa-check'></i>";
    }
 if($stepclass[2]=='current'){
 	$icon1="<i class='fa fa-check'></i>";
    	$icon2="<i class='fa fa-check'></i>";
    }
 if($stepclass[3]=='current'){
 	$icon1="<i class='fa fa-check'></i>";
    	$icon2="<i class='fa fa-check'></i>";
    	$icon3="<i class='fa fa-check'></i>";
    }
 if($stepclass[4]=='current'){
 	$icon1="<i class='fa fa-check'></i>";
    	$icon2="<i class='fa fa-check'></i>";
    	$icon3="<i class='fa fa-check'></i>";
	...
	<div class="stepstat">
		<ul>
			<li class="$stepclass[1]">1--环境检测$icon1</li>
			<li class="$stepclass[2]">2--数据库初始化$icon2</li>
			<li class="$stepclass[3]">3--执行参数$icon3</li>
			<li class="$stepclass[4]">4--完成安装$icon4</li>
		</ul>
