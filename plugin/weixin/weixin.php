...
class wechatCallbackapiTest {
	...
	private function getcontent($keyword, $object = null) {
						...
						if (! empty ( $key ['title'] ) && $key ['title'] != '') {
							...
							while ( $topic = $this->db->fetch_array ( $sql ) ) {
								...
								if (strstr ( $topic ['wburl'], 'http:' )) {
									...
									$kcontent [] = array ("Title" => $topic ['title'], "Description" => $topic ['txtcontent'], "PicUrl" => SITE_URL . $topic ['fmtu'], "Url" => SITE_URL . '?article-' . $topic ['wzid'].'.html' );
		...
		if ($keyword == "账号绑定" || $content == "账号绑定") {
			...
			if ($getone == null) {
				$url = "http://" . $_SERVER ['SERVER_NAME'] . "index.php?account/bind/$openid";
				$content = "<a href='$url'>" . $getone ['username'] . "点击进入账号绑定</a>";
			} else {
				runlog ( 'test', "SELECT * FROM " . DB_TABLEPRE . "user where openid='$openid' limit 0,1" );
	...
	private function receiveText($object) {
		...
		switch ($type) {
			case 'topic' :
				...
				while ( $topic = $this->db->fetch_array ( $query ) ) {
					...
					if ($index == 0) {
						$content [] = array ("Title" => $topic ['title'], "Description" => "", "PicUrl" => $topic ['image'], "Url" => SITE_URL . '?article-' . $topic ['id'] );
					} else {
						$content [] = array ("Title" => $topic ['title'], "Description" => "", "PicUrl" => SITE_URL . $topic ['image'], "Url" => SITE_URL . '?article-' . $topic ['id'] );
					}
		...
		if ($content == "") {
			...
			while ( $topic = $this->db->fetch_array ( $query ) ) {
				...
				$index = strpos ( $topic ['image'], 'http' );
				if ($index == 0) {
					$content [] = array ("Title" => $topic ['title'], "Description" => "", "PicUrl" => $topic ['image'], "Url" => SITE_URL . 'index.php?article-' . $topic ['id'].'.html' );
				} else {
					$content [] = array ("Title" => $topic ['title'], "Description" => "", "PicUrl" => SITE_URL . $topic ['image'], "Url" => SITE_URL . 'index.php?article-' . $topic ['id'].'.html' );
				}
