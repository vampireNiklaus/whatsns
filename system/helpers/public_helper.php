...
function sendsms($key, $mobile, $tpl_id, $tpl_value) {
	...
	$message [] = array ();
	$smsConf = array ('key' => $key, //您申请的APPKEY
...
function dz_segment($title = '', $content = '', $encode = 'utf-8') {
	...
	try {
		...
		$result = $decodedJsonArr ['result'] ['res'] ['wordpos'];
		$data = array ();
		foreach ( $result as $key => $value ) {

			list ( $_k, $_v ) = preg_split ( '/:/', $value );
			if (strlen ( $_k ) >= 4) {
				if (! in_array ( $_k, $data )) {
					array_push ( $data, $_k );
				}
...
function clearhtml($miaosu, $len = '200') {
	...
	$miaosu = cutstr ( str_replace ( '&nbsp;', '', $miaosu ), $len, '...' );
...
if (! function_exists ( 'updateinfo' )) {
	...
	function updateinfo( $user) {
		...
		$pars = array ( 'sitename' => $sitename, 'siteurl' => $siturl, 'email' => $user ['email'], 'version' => ASK2_VERSION, 'release' => ASK2_RELEASE, 'phpos' => PHP_OS, 'phpversion' => phpversion (), 'mysqlversion' =>$CI->db->version(), 'browser' => urlencode ( $_SERVER ['HTTP_USER_AGENT'] ), 'username' => urlencode ( $user ['username'] ), 'phone' => urlencode ( $user ['phone'] ) );
...
function sendmail($touser, $subject, $message, $from = '') {
	...
	$post_data = array ("tousername" => $toemail, 'mailtitle' => $subject, 'mailcontent' => $message );
	$ch = curl_init ();
	curl_setopt ( $ch, CURLOPT_URL, $url );
